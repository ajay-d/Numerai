...
ggplot(gbm.pred.2) + geom_histogram(aes(xgb.2), binwidth = .001)
ggplot(gbm.pred.2) + geom_histogram(aes(xgb.3), binwidth = .001)

###AUC 0.5291
write.csv(gbm.pred.2 %>%
            select(t_id, probability),
          file='gbm_2_3_newdata.csv', 
          row.names = FALSE)